...
def print_help():
    print("""
    fiberStrains.py -a <analysis> -dsr <dsr> -sec <sec>...
...
def yieldpt(X, Y, eps, times):
    for t in range(eps.shape[1]):
        ...
        if any(epst >= 0.002):
            ...
            yieldSummary = pd.DataFrame(
                np.column_stack((coordsYieldedFibers, epsYieldedFibers, [t]*len(coordsYieldedFibers))), 
    ...
    for t in range(eps5.shape[1]):
        ...
        if any(epst5 <= -0.011)  or  any(epst6 >= 0.09):
            ...
            ultSummary5 = pd.DataFrame(
                np.column_stack((coordsUltFibers5, epsUltFibers5, [t]*len(coordsUltFibers5), ["concrete"]*len(coordsUltFibers5))),
                columns = ["Fiber X Coord", "Fiber Y Coord", "Strain", "Timepoint", "Material"]
            )
            ultSummary6 = pd.DataFrame(
                np.column_stack((coordsUltFibers6, epsUltFibers6, [t]*len(coordsUltFibers6), ["steel"]*len(coordsUltFibers6))), 
                columns = ["Fiber X Coord", "Fiber Y Coord", "Strain", "Timepoint", "Material"]
            )
            ...
            plt.scatter(
                np.append(X5, X6), 
                np.append(Y5, Y6), 
                c=np.append(eps5[:, timeYield], eps6[:, timeYield]), 
                vmin=-0.003, 
                vmax=0.003
            )
            plt.colorbar(label="strain")
            plt.scatter(
                np.append(XUltFibers5, XUltFibers6), 
                np.append(YUltFibers5, YUltFibers6), 
                marker='x', color="r", label="Failed Fibers"
            )
            plt.xlabel("Section Horizontal (X) Axis [inches]")
            plt.ylabel("Section Vertical (Y) Axis [inches]")
